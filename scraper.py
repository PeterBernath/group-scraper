...
class CollectPosts(object):
    ...
    def __init__(self, ids=["oxfess"], corpus_file="posts.csv", depth=5, delay=2):
        ...
        with open(self.dump, "w", newline='', encoding="utf-8") as save_file:
            ...
            writer.writerow(["Author", "uTime", "Price", "Text", "Link"])
    ...
    def collect_groups(self, group):
        ...
        with open(self.dump, "a+", newline='', encoding="utf-8") as save_file:
            writer = csv.writer(save_file)
            links = self.browser.find_elements_by_class_name("see_more_link_inner")
            try:
                for link in links:
                    link.click()
            except Exception:
            ...
            for count, post in enumerate(posts):
                ...
                span_element = post.find_elements_by_class_name("oi732d6d")
                if span_element: 
                    analysis.append(html.unescape(span_element[0].get_attribute('innerHTML')).replace('<span>', '').replace('</span>', '').replace(',', '').replace(' Ft', ''))
                ...
                # Creating post link entry
                link_element = post.find_elements_by_class_name("see_more_link")
                if link_element:
                    post_link = link_element[0].get_attribute("href")
                    analysis.append(post_link)
